...
const SearchBar = (props: ISearchBarProps) => {
  ...
  const onSearchChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    if (searchBoxRef && searchBoxRef.current && collection.length < 4)
      searchBoxRef.current.className = `flex ${mainBgColor} ${mainBorderColor} xs:w-40 sm:w-80 h-32 overflow-auto border-2 absolute z-20`;
    if (searchBoxRef && searchBoxRef.current && collection.length > 4)
      searchBoxRef.current.className = `flex ${mainBgColor} ${mainBorderColor} xs:w-40 sm:w-80 h-96 overflow-auto border-2 absolute z-20`;
  };
  ...
  return (
    <div className={`${mainWidth} h-8`}>
      <div
        className={`${mainWidth} ${mainBorderColor} ${mainTextColor} ${mainBgColor} border-2 overflow-hidden flex items-center p-1`}
      >
        <input
