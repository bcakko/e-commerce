...
const UserNav = (props: IUserNavProps) => {
  ...
  useEffect(() => {
    ...
    if (newUserData !== undefined && newUserData.id !== null)
      dispatch(logUserInAction(newUserData.id));
    let favData;
    if (localStorage.hasOwnProperty("favorites")) {
      favData = localStorage.getItem("favorites");
    }
    let newFavData;
    if (favData !== undefined && favData !== null)
      newFavData = JSON.parse(favData);
  ...
  return (
    <div className={`w-24 flex justify-around ${iconColor} items-center`}>
      {isLoggedIn ? (
        <DropdownMenu
          ddTitle={<FaUser className="hover:text-side-color transition-all" />}
          ddList={userLinks[0]}
        ...
        <Link
          to="/auth"
          className="hover:text-side-color transition-all ease-in"
        >
          Sign-in
        </Link>
      )}
      ...
      >
        <LinkWithIcon
          ...
          path="/favorites"
          icon={<FaHeart />}
          className="text-xl p-1 hover:text-side-color
          transition-all"
        />
        <div ref={miniFavListRef} className="hidden">
          <MiniFav
            data_fav={favorites}
            data_style="border rounded overflow-hidden"
          />
        </div>
      </div>
    </div>
